...
export async function createHabit(input: HabitInput): Promise<number> {
  ...
  const result = await db.runAsync(
    `INSERT INTO habits (name, description, emoji, color, frequency_type, frequency_days, sort_order, is_active)
     VALUES (?, ?, ?, ?, ?, ?, ?, 1)`,
    input.name.trim(),
...
export async function updateHabit(id: number, input: HabitInput): Promise<void> {
  ...
  await db.runAsync(
    `UPDATE habits
     SET name = ?, description = ?, emoji = ?, color = ?, frequency_type = ?, frequency_days = ?
     WHERE id = ?`,
    input.name.trim(),
    input.description.trim(),
    input.emoji,
